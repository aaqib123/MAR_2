angular.module('starter.controllers', ['firebase'])
        ...
        .controller('LoginCtrl', function ($scope, auth, $state, store, $firebase) {
            ...
            auth.signin({
                ...
                var a = 'https://martwo.firebaseio.com/users/' + $scope.pro['user_id'];
                var ref = new Firebase(a);
                var sync = $firebase(ref);
                // add ud_date for date update on login
                sync.$update({
                    userdetails: {
        ...
        .controller('DashCtrl', function ($scope, link, $localstorage, locopoco, $window, $http, $filter, store, $firebase, $ionicModal, $interval, $ionicPlatform, $rootScope, $ionicPopup, $ionicLoading) {
            document.addEventListener('deviceready', function () {
                ...
                function tick() { // checks what the "link" in firebase contains, 
                    if (a1.link != 'nothing') { // if nothing = dislpay no bagdge, if has google form link only then display badge         
                    ...
                    if (settin.loc == true) { //check if current location is within 25 mts of saved locations
...
//---------------------------------------SLEEP-------------------------------------------------------------------------
                        ...
                        if ($localstorage.get('sleepnotified') == "yes") {
                            ...
                            if (typeof $scope.notify != 'undefined') {
                                ...
                                angular.forEach($scope.notify, function (localnotifs) {
                                    if (localnotifs.time > $localstorage.get("sleeptime")) {
                                        status = true;
                                    }
                                });
                                if (status == true) {
                ...
                $interval(tick, 20000);// Update every 20 seconds

//                var ab = 'https://accelero.firebaseio.com/moved/';
//                var xc = new Firebase(ab);
//                var zx = $firebase(xc);
                var aiminus1;
                function accelerometerSuccess(acceleration) {
                    ...
                    if (vi >= 1)
                    {
                        ...
                        if ($scope.timegap > 10000 && $localstorage.get('sleepnotified') == "no") { //300000 5 mins moving
                            window.plugin.notification.local.add({
                        ...
                        if ($scope.nomoveforlong > 120000) { //900000 15mins no movememnts at all
                ...
                function distance_notif() {
                    ...
                    if (typeof $scope.notify != 'undefined') {
                        ...
                        if ((status == true) && (settin.notifications == true)) {
                ...
                $scope.togglea = function () {
                    //$scope.notify
                    var aaqib = $localstorage.getObject('locally');
                    delete aaqib[1];
                    $scope.notify = aaqib;
                    if ($localstorage.get('notified') == "yes") {
                        $localstorage.set('notified', "no");
...
//--------------------------------------------------------------------------------------------------------------
        .controller('FriendsCtrl', function ($scope, $http, $filter, store, $firebase, link, $timeout) {
            var a = 'https://martwo.firebaseio.com/users/' + $scope.pro['user_id'];
            var ref = new Firebase(a);
            var act_log = $firebase(ref.child("activity"));
...
//            }
        ...
        .controller('AccountCtrl', function ($scope, $localstorage, auth, $state, store, $firebase, meds, $ionicModal) {
            ...
            var a = 'https://martwo.firebaseio.com/users/' + $scope.pro['user_id'] + '/meds/';
            var ref = new Firebase(a);
            var a1 = 'https://martwo.firebaseio.com/users/' + $scope.pro['user_id'];
            ...
            $scope.newmedicine = function () {
                var newmedurl = 'https://martwo.firebaseio.com/Medicines/';
